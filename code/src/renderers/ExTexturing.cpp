...
ExTexturing::ExTexturing(int width, int height) : Renderer(width, height)
{
	float vertices[]{
...
void ExTexturing::render(float dt)
{
	glBindTexture(GL_TEXTURE_2D, texture);
	glBindVertexArray(VAO);
	program->use();
...
//MÉTODE ALTERNATIU PER CARREGAR CUB
/*
-Dins del constructor
	cube = new Cube();
...
-Dins del render
	...
	//set color
	cube->setColor(glm::vec4(0.6f, 0.1f, 0.1f, 1.0f));

	//translate cube
	cube->setTransforms(
		glm::translate(glm::mat4(), glm::vec3(0.0f, 0.0f, -10.0f)),
		cam);
	...
	//set color
	cube->setColor(glm::vec4(0.1f, 0.1f, 0.6f, 1.0f));
